...
function segment(rows) {
  ...
  for (let windowStart = startTs; windowStart + WINDOW_MS <= endTs; windowStart += WINDOW_MS) {
    ...
    let lon = null;
    if (gpsRows.length > 0) {
      const lats = gpsRows.map((r) => r.lat).sort((a, b) => a - b);
      const lons = gpsRows.map((r) => r.lon).sort((a, b) => a - b);
      const mid = Math.floor(lats.length / 2);
      lat = lats[mid];
      lon = lons[mid];
    }
    const speedSamples = windowRows.filter((r) => r.is_stale === false && r.speed_ms != null).map((r) => r.speed_ms);
    segments.push({ startTs: windowStart, endTs: windowEnd, accel, gyro, lat, lon, speedSamples });
  }
  return segments;
}
const TARGET_HZ = 100;
const TARGET_MS = 1e3 / TARGET_HZ;
function resampleToGrid(rows, startTs, nSamples) {
...
function resampleSegment(accelRows, gyroRows, startTs, windowMs = 1500) {
  const nSamples = Math.round(windowMs / TARGET_MS);
  return [
...
const HP_A = [1, -3.9179078653919865, 5.7570763791180655, -3.7603495076945257, 0.921181929191236];
const LP_B = [0.43284664499029185, 1.7313865799611674, 2.597079869941751, 1.7313865799611674, 0.43284664499029185];
const LP_A = [1, 2.369513007182037, 2.313988414415879, 1.0546654058785672, 0.18737949236818482];
function lfilter(b, a, signal) {
...
function computeSpectrogram(signal) {
  ...
  const sxx = new Float64Array(FREQ_BINS$1 * nFrames);
  ...
  for (let frame = 0; frame < nFrames; frame++) {
    ...
    fft(re, im);
    for (let k = 0; k < FREQ_BINS$1; k++) {
      let power = (re[k] * re[k] + im[k] * im[k]) / SCALE;
      if (k > 0 && k < FREQ_BINS$1 - 1) power *= 2;
      sxx[k * nFrames + frame] = power;
    }
  }
  return { sxx, nFrames };
}
function computeSegmentSpectrogram(accel, gyro) {
  const channels = [
  ...
  const data = new Float64Array(6 * FREQ_BINS$1 * nFrames);
  for (let c = 0; c < 6; c++) {
    data.set(results[c].sxx, c * FREQ_BINS$1 * nFrames);
  }
  return { data, freqBins: FREQ_BINS$1, nFrames };
}
const SPEED_MEAN = [
  11.311052441365556,
  0.5733570943081535,
...
const N_FRAMES = 18;
const N_CHANNELS = 6;
function buildSpectrogramTensor(accel, gyro) {
  const { data } = computeSegmentSpectrogram(accel, gyro);
  const frameSize = FREQ_BINS * N_FRAMES;
...
function buildSpeedTensor(speedSamples) {
...
function buildFeatures(accel, gyro, speedSamples) {
...
async function loadModel(modelPath) {
  const resolved = path.join(process.cwd(), "best_model_speed.onnx");
...
async function matchChunk(chunk) {
  ...
  const timestamps = chunk.map((p, i) => {
    let t = Math.floor(p.timestamp / 1e3);
    if (i > 0) {
      const prev = Math.floor(chunk[i - 1].timestamp / 1e3);
      if (t <= prev) t = prev + 1;
    }
    return t;
  });
...
async function processParsed(rows, modelPath) {
  await ensureModel();
  const segments = segment(rows);
  const gpsPoints = [];
  for (const seg of segments) {
    if (seg.lat === null || seg.lon === null) continue;
    const [accel, gyro] = resampleSegment(seg.accel, seg.gyro, seg.startTs, 1e3);
  ...
  return snapped.filter((p) => p.snapped).map((p) => ({ lat: p.lat, lon: p.lon, quality: p.quality, snapped: p.snapped }));
}
async function processFile(csvPath, modelPath) {
  return processParsed(parseCsv(csvPath));
}
...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      preload: path$1.join(__dirname$1, "../dist-electron/preload.mjs")
    }
  });
